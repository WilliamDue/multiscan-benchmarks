...
filterCoalescedWrite(T* d_in,
    ...
    volatile __shared__ I block[ITEMS_PER_THREAD * BLOCK_SIZE];
    volatile __shared__ I block_aux[BLOCK_SIZE];
    volatile __shared__ I block_size;
    T elems[ITEMS_PER_THREAD];
    bool bools[ITEMS_PER_THREAD];
    I local_offsets[ITEMS_PER_THREAD];
    I global_offsets[ITEMS_PER_THREAD];
    ...
    if (threadIdx.x == blockDim.x - 1) {
        block_size = block[ITEMS_PER_THREAD * BLOCK_SIZE - 1];
    }
    __syncthreads();

    decoupledLookbackScan<I, I, Add<I>, ITEMS_PER_THREAD>(states, block, op, I(), dyn_idx);

    #pragma unroll
    for (I i = 0; i < ITEMS_PER_THREAD; i++) {
        I lid = blockDim.x * i + threadIdx.x;
        global_offsets[i] = block[lid];
    }
    __syncthreads();
    ...
    for (I i = 0; i < ITEMS_PER_THREAD; i++) {
        I lid = blockDim.x * i + threadIdx.x;
        if (bools[i]) {
            block[local_offsets[i] - 1] = global_offsets[i];
        }
    }
    __syncthreads();
    ...
    for (I i = 0; i < ITEMS_PER_THREAD; i++) {
        ...
        I gid = glb_offs + lid;
        if (lid < block_size) {
            d_out[global_offsets[i] - 1] = block_cast[lid];
        }
    }
